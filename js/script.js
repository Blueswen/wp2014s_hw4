...
//以下為canvas的程式碼，基本上不需多動，依據comments修改即可
  ...
  img.src = "img/overlay.png"; //圖像路徑（路徑自己設，且自己加入想要的圖層）
    ...
    var px=0,py=0,tx=200,ty=400;//暫存photo和text的座標
    var mx,my;
    var isStart=false;
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      if(isDragging){ //當拖拉為True時
        ...
        if(!($('#albumSelect').css('display')=='none')){
          profileIMG = document.getElementById("pic");//抓html裡預載入的照片
        }
        else if(!($('#profilePic').css('display')=='none')){
          profileIMG = document.getElementById("preview1");//抓html裡預載入的照片
        }
				...
				if($('#target').val()=='photo'){
					px = canMouseX-profileIMG.width;
					py = canMouseY-profileIMG.height;
				...
				ctx.fillText(inputedText, tx+70,ty+47); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
      }
    }
